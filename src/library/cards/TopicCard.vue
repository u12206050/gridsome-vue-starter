<template>
  <div class="card select-none block rounded-lg shadow-lg border border-gray-300 hover:shadow-md p-4 max-w-lg bg-white text-gray-600" :class="{stacked}">
    <span class="flex items-center justify-between border-b border-gray-200 pb-4">
      ...
      <span class="flex-auto mx-2">
        <h5 class="text-base text-black text-d4slate-dark font-semibold capitalize">{{topic}}</h5>
        <p class="text-sm">Topic</p>
      </span>
      <span @click="following = !following" class="rounded bg-blue-100 text-blue-700 font-semibold py-1 px-2 text-xs"
        :class="{'bg-gray-100 text-gray-700': following}">{{ following ? 'Following' : 'Follow' }}</span>
    </span>
    <g-link :href="path" class="flex flex-col text-base py-4">
      <h3 class="text-lg font-semibold text-d4slate-dark capitalize text-black">{{title}}</h3>
      <g-image class="pointer-events-none w-full rounded object-cover my-4 h-40" :src="image" width="84" height="112"></g-image>
      <p class="block-with-text lines-2 text-d4gray-dark">{{excerpt}}</p>
    </g-link>
    <span class="flex justify-between items-center mt-4">
      <p class="text-d4secondary">Read more</p>
